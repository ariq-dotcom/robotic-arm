...
//*****
#define sensor 7 // sensor untuk mode kerja (belum ditentukan sensor apa)
//*****
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
void loop() {
  // ==== POSISI AWAL / HOME POSITION ====
  // belum 

  // ===== HARD CODE TARGET =====
  float x = 0.0; // koordinat
  float y = 0.0; // koordinat
  float z = 0.0; // koordinat
  float phi = -90; // arah bagian depan
  ...
  // gripper sementara di posisi tengah (90°)
  gripper.write(60);
  delay(1000);
  gripper.write(30);
...
//################################################################
//################################################################
// ==== FUNGSI IK pergelangan terkunci====
void ik_task(float x, float y, float z_target, float phi_deg,
                      float *t0_deg, float *t1_deg, float *t2_deg, 
                      float *t3_deg, float *t4_deg) {
  ...
  // menjaga agar gripper tetap (dikunci)
