...
//sensor
#define sensor 7 // megaatmega2560/ sensor untuk mode kerja (belum ditentukan pakai sensor apa)
...
void gerakKe(float x, float y, float z, float phi) {
  ...
  while (true) {
    ...
    sekarang_s0 += (s0 - sekarang_s0) * k;
    sekarang_s1 += (s1 - sekarang_s1) * k;
    sekarang_s2 += (s2 - sekarang_s2) * k;
    sekarang_s3 += (s3 - sekarang_s3) * k;
    sekarang_s4 += (s4 - sekarang_s4) * k;
    ...
    // ===== CEK SUDAH SAMPAI =====
    if (abs(s0 - sekarang_s0) < 0.02 &&
        abs(s1 - sekarang_s1) < 0.02 &&
        abs(s2 - sekarang_s2) < 0.02 &&
        abs(s3 - sekarang_s3) < 0.02 &&
        abs(s4 - sekarang_s4) < 0.02) {
      break;
    }
